...
-- ============================================================
--
--  Дневник сна рядом с трекером добавок: одна запись на ночь,
--  своя у каждого человека. Записать можно руками или перенести
--  файлом из приложения часов, кольца или браслета — столбец
--  source помнит, откуда ночь пришла.
--
--  Здесь же появляется раздел у записей базы знаний: знание с
--  разделом «sleep» показывается клиенту в кабинете как
--  рекомендация Professor Python по сну.
...
select "Что проверяем", "Статус" from (
  ...
  union all select 15,
    ...
    case when to_regclass('public.supplement_intakes') is not null
      then '✅ есть' else '❌ НЕТ' end
  union all select 16,
    'Трекер сна (sleep_entries)',
    case when to_regclass('public.sleep_entries') is not null
      then '✅ есть' else '❌ НЕТ' end
  union all select 17,
    'Раздел у знаний (assistant_knowledge.topic)',
    case when exists (
      select 1 from information_schema.columns
      where table_schema = 'public' and table_name = 'assistant_knowledge'
