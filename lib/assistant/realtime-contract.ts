...
export const voiceCopy = {
  ru: {
    ...
    staffDisclosure: "Ваши слова и ответы Анхама появляются здесь текстом и сохраняются в истории. Это голос ИИ. При запуске вы разрешаете передавать речь голосовому сервису, а по вашему вопросу — данные сайта и клиентов, включая переписку, анкеты и имеющиеся медицинские записи. Для интернет-поиска общий запрос передаётся внешнему поисковому сервису. Ответы и ссылки сохраняются здесь. Анхам читает сведения, но не изменяет их. Аудиофайл не записывается.",
    disclosure: "Вы услышите голос ИИ. Нажимая кнопку, вы разрешаете передать речь внешнему голосовому сервису и сохранить реплики текстом. Платформа не записывает аудиофайл. Это тестовый режим: не сообщайте медицинские и личные данные. Документы и прошлая переписка в голос не передаются.",
  ...
  en: {
    ...
    staffDisclosure: "Your words and Anham’s replies appear here as text and are saved in history. This is an AI voice. Starting allows speech to be sent to the voice service and, when you ask, site and client data, including correspondence, questionnaires and existing medical records. For web searches, a public-topic query is sent to an external search service. Answers and links are saved here. Anham reads information but does not change it. No audio file is recorded.",
    disclosure: "You will hear an AI voice. Starting allows speech to be sent to an external voice service and turns to be saved as text. The platform does not record an audio file. This is a test mode: do not share medical or personal information. Documents and previous conversations are not sent to voice.",
