...
2026-09-18 source-derived deskew candidate: commit dec973e adds a reversible normalized-coordinate derivative before laboratory row association. The estimator uses only OCR token polygon edges (never fixture angle or Gold), records slope/sample/MAD plus source and derivative geometry hashes, retains every original token polygon/span, and rejects unstable, excessive or out-of-page transforms. All candidates remain NEEDS_REVIEW; 0 VERIFIED by construction. Local tilted-table regression passes; full suite 1854 passed, 1 skipped; TypeScript, ESLint, security 6/6/inventory, diff check and npm audit pass. Vercel Preview is Ready. The new live three-raster repeat is pending one admin sign-in on the stable branch alias, so robust-layout acceptance remains NOT CLOSED. Evidence: docs/ankh/source_derived_deskew_2026_09_18.md.
