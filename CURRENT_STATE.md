...
Dedicated `anham-clinical-staging` preview is READY for exact remote `454cd0642487360a5cdbf8d92979b9faeb4bb890`, deployment `dpl_Fkvpy2YbndZHNvsHjvAB7771hDhS`; deployed document bundle confirms isolated DB `thylrayzjczsxlyqhtfc`. Browser fallback was owner-approved. Home, login RU/EN and guest admin redirect pass. The owner created a separate confirmed staging client account and signed in. Initial onboarding failed on missing physical `profiles.country_code`; narrow stage-only migration `20260924044828_restore_staging_profiles_country_code_20260924` restored the column/constraint and requested schema reload. The owner then successfully submitted; stage readback confirmed one Case, one submission and two consents. No resubmission is needed. The owner uploaded `pmc-synthetic-lab-20260924.pdf` into that Case; the two-page fictional PDF was opened and verified. Original storage and job registration passed. Three attempts failed at the external header reader with only `HEADER_READER_UNAVAILABLE` retained. Job/document are `failed`, with zero extraction/analysis rows; hosted steps 4–11 remain open. Anthropic key is present in the Preview settings and the model ID is current, but the specific response class remains unknown. Production was not migrated/deployed.

The owner found that a failed onboarding submit cleared her answers. A local form fix dispatches the submitted snapshot without automatic form reset; focused retry and related tests total 8 passed, with TypeScript and targeted lint passing. It is not in the deployed `454cd064` build and has no hosted verification yet. It does not persist sensitive draft answers in browser storage.

The deployed client status currently tells her `Нужен новый файл` for a service failure even though the original is intact. A local-only repair changes that state to `Сбой обработки` / `Processing failed`, distinguishes bounded provider failure classes in the private job error, and rejects a provider policy refusal as extraction evidence. The latest relevant suite passes 280/280, synthetic benchmark 1/1, TypeScript, targeted ESLint, security check and diff check. No new build was deployed or provider rerun claimed for this repair.

Staging's originally disconnected Git was temporarily connected to create the exact preview. Automatic approval review rejected restoring disconnection without explicit owner approval. The owner explicitly approved removing it on 2026-09-24; the staging Git settings now show no connected repository. New commits may be pushed without automatically deploying this dedicated staging project. The deployed head remains `454cd064` until a separately verified isolated deployment. Do not call all eleven steps accepted or claim full v2 skill-runtime activation. [Exact evidence and next gate](docs/ankh/pmc_document_chain_2026_09_24.md).
