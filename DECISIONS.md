...
Owner-approved browser fallback deployed exact `454cd064` to dedicated clinical staging (`dpl_Fkvpy2YbndZHNvsHjvAB7771hDhS`, READY), with deployed bundle binding to isolated `thylrayzjczsxlyqhtfc`. The owner then created a confirmed staging client account and signed in. Initial onboarding exposed absent `profiles.country_code`; stage-only migration `20260924044828_restore_staging_profiles_country_code_20260924` restored the existing app contract without replaying the unrelated historical data update. The owner's next submission succeeded; one Case, one onboarding submission and two consents were read back. Browser file selection stalled, leaving zero uploaded documents. The failed-submit draft-retention repair is local and has not reached this deployment. Synthetic upload/provider/approval/publication acceptance are still required. Do not work around this by testing against production, changing credentials or impersonating Karen.

Later staging evidence supersedes the zero-document state: the owner uploaded one two-page synthetic PDF. Storage/opening/registration succeeded, but the three header-reader attempts failed; extraction and analysis did not start. No reupload or questionnaire resubmission is needed. The local diagnostic change stores only fixed error categories in the private job row, never raw provider responses or document text; a provider refusal cannot become source evidence. The local client status distinguishes service failure from an unreadable source in RU/EN. Provider root cause and hosted verification remain pending deployment, then an authorized rerun of that existing isolated source.

The staging Git connection was temporarily enabled for exact-commit deployment. Automatic review refused its removal without explicit owner approval. The owner gave that explicit approval on 2026-09-24; the isolated staging project's Git settings now show no connected repository. Do not infer a new staging deployment from a Git push; deploy and verify that target explicitly.
...
## D-069 — Email alerts reveal only the existence of a client message (2026-09-17)

Send transactional email for new staff-authored Support and Professor Python
messages to the registered account owner's profile email. Resolve the recipient
and locale only on the server. Email identifies the channel and links to the
authenticated cabinet, but never includes free-text message content, patient
identity, Case metadata or medical information. Use the existing
`notification_events` ledger plus a channel/message-ID dedupe key. Delivery is
best-effort after the site message is stored, so a mail-provider failure cannot
erase or block the canonical conversation. Automated Anham turns, marketing,
SMS and reminder scheduling are outside this increment.
