...
Owner review found one mixed template/handwriting failure: a bare printed `норма` before entered pancreatic dimensions was treated as a selected value. The general regression gate requires an explicit visual selection signal for leading template state words in dimension/structure fields. Without it, matching OCR text remains review-only; concrete trailing assessments and visibly selected options are not suppressed.
