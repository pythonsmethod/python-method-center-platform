...
Если пользователь завершает голос во время текущей делегации, browser abort больше
не отменяет backend. Next.js удерживает задачу в пределах времени функции, а
результат записывается в тот же `assistant_messages` scope и появляется в
текстовом чате. Подробности: `docs/ankh/voice_background_continuation.md`.
...
аудио. Уже начатая делегация продолжает работу после закрытия голосового request
и пишет ответ в чат, но только в пределах общего `maxDuration=300`; задачи за
пределами времени жизни server function не являются durable jobs.
