...
The dedicated isolated project `anham-clinical-staging` has no deployment of this branch. Its last observed deployment `dpl_CPwYYorG1UcaJB3ETAqqyn8qrM6V` still points to old `b0503cbb7a9c198cb87a119940cb985667c21990`. Direct Vercel deployment connector returns `Tool deploy_to_vercel not found`; CLI is logged out. Read-only project lookup works when the underlying `idOrName` parameter is supplied alongside the advertised `projectId`; it does not provide deployment or environment writes. Git CLI push lacks credentials; the connected GitHub write API saved and read back the branch. No production database migration, main merge or production deployment was performed.
