...
vi.mock("@/lib/supabase/service", () => ({ createSupabaseServiceClient: () => ({ rpc: async () => ({ data: "pmc1:" + "a".repeat(32), error: null }), from: (table: string) => {
  ...
  const query = { select: () => query, eq: () => query, is: () => query, order: () => query, limit: () => query, range: () => query, maybeSingle: () => query, in: () => query,
