...
import { AssignVolunteerForm } from "@/components/delivery/AssignVolunteerForm";
...
export default async function FulfillmentPage() {
  ...
  return <div className="page-shell page-shell--wide"><PageHeader eyebrow={ru ? "Рабочее место Анны" : "Anna's workspace"} title={ru ? "Доставки" : "Deliveries"} description={ru ? "Адреса, состояния, документы с трек-номерами и комментарии волонтёров." : "Addresses, statuses, tracking documents, and volunteer comments."} />
    ...
    <div className="fulfillment-list">{data.tasks.map(task => <div key={task.id}><DeliveryTaskCard task={task} locale={locale} documentUrl={task.documentUrl} admin /><AssignVolunteerForm taskId={task.id} currentId={task.volunteer_id} volunteers={data.volunteers ?? []} locale={locale} /></div>)}</div>
