...
export function RealtimeVoice({ locale, scope, caseId, disabled, onActive, onExchange, onTranscript }: Props) {
  ...
  const controller = useRef<RealtimeBrowser | null>(null);
  ...
  function start() {
    if (running.current || disabled || pending.current.length || choice.loading || previewBusy) return;
    setChoosing(false);
  ...
  return <>
    <button className="assistant-voice-launcher" type="button" aria-label={copy.start} title={copy.start}
      aria-haspopup="dialog" aria-expanded={open} disabled={(disabled && !active) || choice.loading}
    ...
    {open ? createPortal(<dialog className="anham-call" ref={dialogRef} aria-labelledby={`${descriptionId}-title`}
      ...
      <div className="anham-call__layout" data-state={state}>
        ...
        <footer className="anham-call__footer">
          ...
          {saving !== "idle" ? <p role="status">{copy[saving]} {saving === "saveError" ? <button type="button" onClick={() => void flush()}>{copy.retrySave}</button> : null}</p> : null}
