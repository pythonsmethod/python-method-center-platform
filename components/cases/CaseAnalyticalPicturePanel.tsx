...
function AllEvidenceDetails({ caseId, locale, items, canConfirm, labels, categoryLabel }: {
  ...
  return <details onToggle={(event) => setOpen(event.currentTarget.open)}>
    ...
    {open ? <ul className="status-list">{items.map((item) => <li key={`all-${item.id}`}><strong>{categoryLabel(item.category)} · {item.section} · {item.label}</strong><br />{item.value ?? "—"}{item.alternateValue !== null ? ` ↔ ${item.alternateValue}` : ""} · <Link href={`/admin/documents/${item.documentId}/view${item.provenance.page ? `?page=${item.provenance.page}` : ""}`} target="_blank">{labels.open}</Link>{canConfirm ? <EvidenceReviewControls caseId={caseId} item={item} labels={labels} locale={locale} /> : null}</li>)}</ul> : null}
  </details>;
}
...
export function CaseAnalyticalPicturePanel({ caseId, locale, result, canConfirm }: {
  ...
  const t = ru ? {
    ...
    documents: "Документы и источники", facts: "Хронология фактов", extracted: "Очередь исключений", extractedIntro: "Здесь показаны спорные чтения и важные строки. Все исходные строки доступны ниже. Совпадение чтений не присваивает фактам VERIFIED.", allEvidence: "Архив всех извлечённых строк", comparisons: "Сравнения по времени", conflicts: "Противоречия", missing: "Чего не хватает", review: "Проверка Карен", notes: "Замечания Карен", noDocuments: "В кейсе нет доступных документов.", noFacts: "Структурированные факты пока отсутствуют. Документы перечислены выше и не считаются обработанными автоматически.", noComparisons: "Для обоснованного сравнения недостаточно сопоставимых датированных данных.", none: "Расхождений для ручной проверки не выявлено.", open: "Открыть источник", dateMissing: "дата отсутствует", sourceOnly: "сопоставлено · только источник", needsReview: "нужна проверка", potential: "возможное изменение", noChange: "подтверждённой динамики нет", notComparable: "несопоставимо", insufficient: "недостаточно данных", provenance: "Точность ссылки: документ; identity/header, страница и токены не подтверждены в этом сохранённом слое.", notePlaceholder: "Замечание к целостной картине…", saveDraft: "Сохранить черновик", confirm: "Подтвердить замечание", draft: "Черновик", confirmed: "Подтверждено Карен", limitations: "Границы", confirmEvidence: "Подтвердить", correctEvidence: "Исправить", rejectEvidence: "Отклонить", correction: "Исправленный текст (обязателен для исправления)", saved: "Решение сохранено", criticalProgress: "Критические пункты", approvalBlocked: "Утверждение заключения заблокировано до проверки всех сомнительных пунктов.", machineMatched: "Не требуют ручного решения", archivedRows: "Технических строк в архиве", manualExceptions: "Требуют решения Karen",
  } : {
  ...
  const reasonLabel = (code: CaseAnalyticalPicture["comparisons"][number]["reasonCode"]) => ({
    ...
    MISSING_CONTEXT: ru ? "Не указаны необходимые условия сравнения: материал или метод." : "Required comparison context is missing: specimen or method.",
  ...
  const missingLabel = (item: CaseAnalyticalPicture["missingContext"][number]) => ({
    ...
    PAGE_TOKEN_PROVENANCE: ru ? "Для части прежних извлечений доступна ссылка только на документ." : "Some earlier extractions have document-level references only.",
